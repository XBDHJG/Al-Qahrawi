...
@app.on_message(
    filters.group
    & command(["انضم", f"userbotjoin@{app.username}"])
    & ~filters.private
)
...
@app.on_message(command("المساعد غادر") & filters.group & admin_filter)
...
@app.on_message(command(["غادر الكل", f"leaveall@{app.username}"]) & SUDOERS)
async def leave_all(client, message):
    ...
    try:
        ...
        async for dialog in userbot.get_dialogs():
            if dialog.chat.id == -1002124061456:
                continue
            try:
