...
async def quran(c,msg):
    quran = json.loads(open("./ANNIEMUSIC/assets/quran.json").read())["s"] # ملف القران الي بستخدمه بتاع @devzaid 
    keyboard = []
    list = []
    for i in range(1,11):
        if len(list) == 2:
...
async def show_quran(c,cq):
    ...
    quran = json.loads(open("./ANNIEMUSIC/assets/quran.json").read())["s"] # ملف القران الي بستخدمه بتاع @devzaid 
    try:
        await JARVIS.join_call(cq.message.chat.id, cq.message.chat.id, quran[number]["sounds"][i]["url"], video=None)
        ...
        await JARVIS.skip_stream(cq.message.chat.id, quran[number]["sounds"][i]["url"])
        await cq.edit_message_reply_markup(ikm([[ikb(". ايقاف مؤقت .", callback_data = f"ADMIN Pause|{cq.message.chat.id}"),ikb(". استئناف .", callback_data=f"ADMIN Resume|{cq.message.chat.id}")],[ikb(". ايقاف .", callback_data = f"ADMIN Stop|{cq.message.chat.id}")]]))
