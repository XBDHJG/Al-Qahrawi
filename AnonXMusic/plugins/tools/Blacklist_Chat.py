...
from ANNIEMUSIC.mongo.blacklist_db import (
...
from ANNIEMUSIC.utils.permission import adminsOnly, list_admins
from ANNIEMUSIC.misc import SUDOERS as SUDO
...
@app.on_message(filters.command(["منع"], "") & ~filters.private)
@adminsOnly("can_restrict_members")
async def save_filters(_, message):
    if len(message.command) < 2:
        return await message.reply_text("طريقه الاستخدام:\nمنع + الكلمه او الجمله")
    word = message.text.split(None, 1)[1].strip()
    if not word:
        return await message.reply_text("**طريقه الاستخدام**\nمنع + الكلمه او الجمله")
    chat_id = message.chat.id
    await save_blacklist_filter(chat_id, word)
    await message.reply_text(f"__**تم اضافه كلمه لقائمة الممنوع\n {word}**__")


@app.on_message(filters.command(["الممنوع"], "") & ~filters.private)
@capture_err
...
@app.on_message(filters.command(["المسموح"], "") & ~filters.private)
@adminsOnly("can_restrict_members")
